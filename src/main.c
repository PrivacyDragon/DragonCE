...
uint24_t about2(uint24_t score){
    gfx_FillScreen(255);
    gfx_PrintStringXY("DRAGONCE, a virtual dragon on your calculator!",1,1);
...
uint24_t UPDATEVAR(uint24_t score){
...
int main(void)
{
    uint24_t score = OPENVAR(); //get the score from the appvar
    ...
    lines(level); //draw the rest of the screen
    score = program_run(score);
    UPDATEVAR(score); //update the score appvar with the new score
    gfx_End();
}
